...
<div style="display:flex; align-items:center; gap:12px; margin-bottom:32px;">
    ...
    <div>
        <h2 style="font-family:'Playfair Display',serif; font-size:1.3rem; font-weight:700; color:var(--text-primary); margin:0;">Editar Noticia</h2>
        <p style="font-size:0.8rem; color:var(--text-secondary); margin:3px 0 0;">Editando: <strong style="color:var(--accent);">{{ $news->title }}</strong></p>
    </div>
</div>
            ...
            <form action="{{ route('news.update', $news) }}" method="POST">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <textarea name="content" rows="5"
                              ...
                              onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='#E2E8F0'">{{ old('content', $news->content) }}</textarea>
                </div>
                ...
                <div style="margin-bottom:28px;">
                    ...
                    <select name="cycles[]" multiple
                        ...
                        @foreach($cycles as $cycle)
                            {{-- Marca los ciclos que ya tenía la noticia --}}
                            <option value="{{ $cycle->id }}" {{ in_array($cycle->id, old('cycles', $news->cycles->pluck('id')->toArray())) ? 'selected' : '' }}>
